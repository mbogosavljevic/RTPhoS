...
#############################
#
def init_datapoins(tempfile, now):
...
##############################
# plot_datalog
def plot_datalog(fig, ax1, ax2, ax3, ax4, ax5, \
    ...
    return line3a, line3b, line4, line5, errorlines3a, errorlines3b, errorlines4
...
def append_datapoints(columns, xdata, yrawtarget, yrawtargeterr, \
    ...
    print "RTPhoS: datapoint at time %s added" % str(newx)
...
######
     try: 
        ...
        command = "cp -f "+args.logfile+" "+args.logfile+"_tmp"
        os.system(command)
        tempfile = args.logfile+"_tmp"
        now = datetime.utcnow()
        xdata, yrawtarget, yrawtargeterr, yrawcomp, yrawcomperr, \
        ydflux, ydfluxerr, yseeing, UTCtimes = init_datapoins(tempfile, now)
        ...
        line3a, line3b, line4, line5, errorlines3a, errorlines3b, errorlines4 = \
                     plot_datalog(fig, ax1, ax2, ax3, ax4, ax5, \
                                  xdata, yrawtarget, yrawtargeterr, yrawcomp, \
                                  yrawcomperr, ydflux, ydfluxerr, yseeing)
        fig.canvas.show()
        npoints_current = len(xdata) 
